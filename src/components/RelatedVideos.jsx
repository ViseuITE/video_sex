...
function RelatedVideos({ relatedVideos }) {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-white font-bold text-xl">Related videos</h2>
        <ul className="pt-1 grid lg:grid-cols-1 md:grid-cols-3 grid-cols-2 md:gap-6 gap-4 lg:gap-0 lg:pt-0">
          <li>
            <video loop autoPlay className="">
              <source src={require(`../assets/video2.webm`)} type="video/mp4" />
          ...
          <li>
            <video loop autoPlay className="">
              <source src={require(`../assets/video1.webm`)} type="video/mp4" />
              Your browser does not support the video tag.
            </video>
          </li>
          {relatedVideos?.length > 0 &&
            relatedVideos.map((e, i) => (
              <li
                ...
                className="cursor-pointer group z-20 bg-[#0A0511] lg:py-4"
                key={i + e.id}
              >
                <Link
                  to={`/details/${e.id}`}
